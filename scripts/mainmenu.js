...
function menu(DT) {
    ...
    if (mainmenu) { menufadeout-- }

    for (let i = 0; i < 2; i++) {
    if (i == 1){ctx_text.fillStyle = "#00ffc850"
                ctx_text.font = "19.5px Courier New"
                X= X + 10
        }
    if (DT > 500) { ctx_text.fillText("======================================================================", X, Y - 500); boxlength = 100 }
    ...
    if (DT > 7000) { ctx_text.fillText("#1: ACQUIRE A BIOLOGICAL SAMPLE FROM THE PLANT MATTER WITHIN THE DERILICT.", X, Y + 60); boxlength += 40 }
    ...
    if (DT > 10000) { ctx_text.fillText("           [TRANSMISSION HANDSHAKE COMPLETED SUCCESSFULLY]", X, Y + 200); boxlength += 40 }
    }
